...
function Dice.new_enemy_die(faces, color, sprite)
	local enm_die = {};
	enm_die.name = "generic enemy die";
	enm_die.faces = faces;
	enm_die.color = color;
	enm_die.curr_face = 1;
	enm_die.sprite = sprite;
	enm_die.position = {};
	enm_die.position.x = 9;
	enm_die.position.y = 4.5;
	enm_die.position.origin_x = 9;
	enm_die.position.origin_y = 4.5;
	enm_die.has_click_focus = false;
	enm_die.type = "dice";
	enm_die.has_box = false;
	enm_die.box = nil;
	enm_die.spent = false;
	enm_die.original_ind = 0;
	enm_die.acumulated_dt = 0;
	function enm_die.roll(self, sched) 
	...
	function enm_die.answer_message(self, sender, message) 
	...
	function enm_die.update(self, dt)
		...
		local enm_die_x = math.floor(playable_bounds.arena.left + (self.position.x/16)*(playable_bounds.arena.right-playable_bounds.arena.left));
		local enm_die_y = math.floor(playable_bounds.arena.top + (self.position.y/9)*(playable_bounds.arena.bottom-playable_bounds.arena.top));
		local target_x = 0;
		local target_y = 0;
		...
		elseif self.has_box == true then
			...
			target_y = math.floor(playable_bounds.arena.top + (self.position.origin_y/9)*(playable_bounds.arena.bottom-playable_bounds.arena.top));
				...
				local x_diff = (target_x - enm_die_x);
				local y_diff = (target_y - enm_die_y);
				...
				self.position.x = (enm_die_x+x_change-playable_bounds.arena.left)/(playable_bounds.arena.right-playable_bounds.arena.left)*16;
				self.position.y = (enm_die_y+y_change-playable_bounds.arena.top)/(playable_bounds.arena.bottom-playable_bounds.arena.top)*9;
			end
	end
	function enm_die.answer_mouse_up(self, x, y, button, sched, my_i)
		return false;
	end
	function enm_die.answer_mouse_down(self, x, y, button, sched, my_i)
	...
	function enm_die.draw(self) 
		love.graphics.setColor(self.color.r, self.color.g, self.color.b);
		local enm_die_x = math.floor(playable_bounds.arena.left + (self.position.x/16)*(playable_bounds.arena.right-playable_bounds.arena.left)-self.sprite:getWidth()*pixel_size/2);
		local enm_die_y = math.floor(playable_bounds.arena.top + (self.position.y/9)*(playable_bounds.arena.bottom-playable_bounds.arena.top));
		love.graphics.draw(self.sprite, enm_die_x, enm_die_y-self.sprite:getHeight()*pixel_size/2,0,pixel_size,pixel_size);
		love.graphics.setFont(Dice.enm_die_font);
		local width_text = Dice.enm_die_font:getWidth(self.faces[self.curr_face].value.."");
		...
		love.graphics.printf(self.faces[self.curr_face].value.."", enm_die_x, math.floor(enm_die_y-Dice.die_font:getHeight()/2), self.sprite:getWidth()*pixel_size, "center");
	end
	return enm_die;
end
...
function Dice.new_die(faces, color, sprite)
	...
	function die.draw(self) 
		...
		local die_x = math.floor(playable_bounds.arena.left + (self.position.x/16)*(playable_bounds.arena.right-playable_bounds.arena.left));
		local die_y = math.floor(playable_bounds.arena.top + (self.position.y/9)*(playable_bounds.arena.bottom-playable_bounds.arena.top));
		love.graphics.draw(self.sprite, die_x-self.sprite:getWidth()*pixel_size/2, die_y-self.sprite:getHeight()*pixel_size/2,0,pixel_size,pixel_size);
		...
		self.faces[self.curr_face].quirk:draw(die_x-self.sprite:getWidth()/2,die_y-self.sprite:getHeight()/2, self.faces[self.curr_face]);
	end
	return die;
end
